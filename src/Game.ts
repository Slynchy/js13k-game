...
export class Game {
    ...
    private BlockInputReasons: Record<string, boolean> = {};
    ...
    constructor(_engine: Engine, _level: LevelFormat) {
        ...
        this.ENGINE_REF.scene.push(this.TARGET_ELEMENT = Game.createTargetElement(this.GAME_STATE.targetScore));
        this.ENGINE_REF.scene.push(this.SCORE_ELEMENT = Game.createScoreElement());
        ...
        this.UP_BLOCKS.push(...Game.createUpBlocks(this.GAME_STATE.upAllowance, this.ENGINE_REF.getAsset("./assets/button_y.png")));
    ...
    private static createTargetElement(targetScore: number): Text {
        return new Text({
            x: (MP.x * 2) + (-75),
            y: 45,
            anchor: {x: 1, y: 0},
    ...
    private static createScoreElement(): Text {
        return new Text({
            x: (MP.x * 2) + (-75),
            y: 25,
            anchor: {x: 1, y: 0},
    ...
    private static createUpBlocks(upAllowance: number, upBlockImg: HTMLImageElement): Sprite[] {
        const res: Sprite[] = [];
        for(let i: number = 0; i < upAllowance; i++) {
    ...
    private static createGridElements(_level: LevelFormat, _buttonImg: HTMLImageElement, _onBtnPress: Function): IGrid {
        ...
        for(let y: number = 0; y < _level.height; y++) {
            retVal.push([]);
            for(let x: number = 0; x < _level.width; x++) {
                const yOffset: number = 40;
                let textObj: Text;
                const position: {x: number, y: number} = {
                    x: (MP.x - ((20 * _level.width) / 2)) + (20 * x) - 90,
                    y: yOffset + (20 * y)
                };
    ...
    private isAllowedToInput(): boolean {
        ...
        Object.keys(this.BlockInputReasons).forEach((e: string) => {
            if(this.BlockInputReasons[e]) {
    ...
    private async onButtonPress(_x: number, _y: number): Promise<void> {
        ...
        this.BlockInputReasons.UpdatingGameState = true;
        ...
        delete this.BlockInputReasons.UpdatingGameState;
        //
    ...
    private static getText(x: number, y: number, _state: LevelFormat): string {
        return _state.contents.grid[y][x] === GridTypes.EMPTY ?
            ...
            :
            TYPES[(_state.contents.grid[y][x] + TYPE_OFFSET)];
    }
